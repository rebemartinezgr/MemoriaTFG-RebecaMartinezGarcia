...
Este proyecto ha consistido en la creación de una aplicación web adaptativa y multi idioma de venta online de tours privados y de un sistema de gestión de reservas.
...
La aplicación se ha alojado en una máquina externa, bajo un servidor web Apache y es accesible de forma pública \footnote{\url{http://dev.dbcntours.com}}.

La aplicación permite al usuario consultar el catálogo de tours ofrecidos, sus características e imágenes y la disponibilidad tanto horaria como lingüística en tiempo real de cada tour. El usuario puede también formalizar su reserva a través de los diferentes métodos de pago que se ofrecen. Además permite al administrador de reservas, a través de un panel de administración, gestionar las diferentes reservas, los guías y sus disponibilidades y los horarios e idiomas de cada tour, entre otras funcionalidades.

El proyecto se ha llevado a cabo bajo la petición de un cliente real, la empresa DB Tours, la cual se encuentra especializada en tours privados y a medida en la ciudad de Barcelona.
DB Tours hará de esta aplicación, su nueva web oficial así como su sistema de gestión de reservas principal.

El cliente ha estado involucrado desde el inicio de la fase de análisis, durante el proceso de desarrollo y hasta la fase de pruebas final.
...
The application has been hosted on an external machine, under an Apache web server and it has public access \footnote{\url{http://dev.dbcntours.com}}.

The application allows the user to consult the catalog of offered tours, their characteristics and images, and the schedules and linguistic availability in real time of each tour. The user can also confirm their reservation through the different payment methods that are offered.

The application allows the reservation manager, through an administration panel, to manage the existing bookings, the guides and their availability and the schedules and languages of each tour, among other functions.

The project has been developed under the request of a real client, the company DB Tours, which is specialist in private and tailor-made tours in Barcelona.
This application will become the official website of the company DB Tours, as well as its main booking management system.

The client has been involved from the beginning of the analysis phase, during the development process until the final testing phase.
...
Este primer capítulo introduce una presentación del trabajo realizado, las motivaciones que han empujado su desarrollo y las tecnologías utilizadas. Así mismo se presenta la estructura del propio trabajo ofreciendo una visión general de su contenido.
...
A continuación se describen las tecnologías más utilizadas para la realización de la aplicación.

\subsection{Magento como plataforma e-commerce} 
\label{sec:magento}
...
\subsubsection{Jerarquía de tiendas}
...
\subsection{Tecnologías front-end} 
\label{sec:web-front}
...
\subsection{Tecnologías back-end} 
\label{sec:web-back}
...
\textit{PhpStorm}~\cite{phpstorm} es un IDE (Integrated Development Environment) comercial multiplataforma para \emph{PHP} creado por JetBrains. \emph{PhpStorm} proporciona un editor para \textit{PHP}, \textit{HTML} y \textit{JavaScript} con asistencia de código inteligente que ayuda a la prevención de errores y refactorizaciones automatizadas para código \emph{PHP} y \emph{JavaScript}. 
...
\label{subsec:github}
\textit{GitHub}~\cite{github} es una plataforma online de alojamiento del código basada en el sistema de control de versiones \emph{Git}, que añade servicios a lo que ya ofrece \textit{Git} nativamente.
Algunas de las principales funcionalidades que ofrece GitHub son:

\begin{itemize}
\item Edición en línea de archivos.
\item Proyectos wiki.
\item Bifurcación de repositorios \emph{(forking)}.
...
\begin{itemize}
  \item \textbf{Capítulo~\ref{chap:intro}. Introducción:} Este capítulo contiene una presentación del trabajo, las motivaciones que han empujado a su desarrollo, las tecnologías utilizadas y la propia estructura de la memoria.
  ...
  \item \textbf{Capítulo~\ref{chap:resultados}. Resultados:}  En este capítulo se listan los resultados obtenidos.
   ...
   \item \textbf{Apéndice~\ref{app:publicacion}. Publicación: } En éste apéndice se ofrecen los enlaces principales tanto a la aplicación como a los repositorios que contienen el código y la presente memoria.
    \item \textbf{Apéndice~\ref{app:code}. Código: } En éste apéndice se presentan algunos de los fragmentos de código más importantes en la implementación de la aplicación.
    \item \textbf{Apéndice~\ref{app:server}. Configuración y Despliegue: }  En éste apéndice se describen los pasos y comandos necesarios para la configuración del servidor externo así como el despliegue de la aplicación en el mismo.
   \end{itemize}
...
A continuación se describen las tres fases en las que se divide el desarrollo de la aplicación y se desglosan las tareas necesarias para alcanzar los objetivos descritos anteriormente.
\subsection{Fase I}
...
Sistema de generación automática de entidades \emph{Tour Events}.
...
Creación de contenidos estáticos por idiomas: \emph{home}, menú, pie de página, páginas de condiciones, zonas de recogida, tours a medida.

\item \textbf{Desarrollo:}

Maquetación de contenidos estáticos: \emph{home}, menú, pie de página, zonas de recogida, tours a medida.

Sistema de generación de \emph{tours events} manual desde el panel de administración para cada producto.
...
Automatización de un sistema de \emph{backups} en el servidor.
...
\item Tipo \emph{tour event}: Es una opción personalizada que se ha desarrollado con el fin mostrar dinámicamente al usuario las opciones de fecha, hora e idioma existentes, como se puede ver en la figura~\ref{fig:datelanguagefront}.
...
Esta sección presenta el enlace entre la base de datos y la aplicación, así como la estructura de base de datos utilizada por la aplicación en lo que al sistema de reservas se refiere, es decir, las nuevas entidades creadas para dicho propósito, dejando al margen el resto de entidades que utiliza Magento para su funcionamiento por defecto.
\label{sec:db}

\subsection{Conexión}
\label{subsec:db-enlace}
Los datos de conexión de la aplicación con la base de datos, se definen en el archivo de configuración \emph{env.php} que se encuentra localizado en la ruta app/etc/env.php del código. Éste archivo no se encuentra bajo el sistema de control de versiones de Git por seguridad.

A continuación se muestra parte de dicho archivo en el que se pueden ver los valores que han sido necesarios definir:
\begin{verbatim}
 <?php return [
	'db' => [
        'table_prefix' => '',
...
\subsection{Generación automática de \emph{Tour Events}} 
\label{sec:tourevents}
Cada \emph{Tour Event} es una versión de un producto concreto para una fecha y hora determinada, que potencialmente podría ser reservada, siempre y cuando la disponibilidad de un guía lo permitiese.
...
\begin{figure}
  ...
  \caption{Generación \emph{Tour Events}: Configuraciones}
  \label{fig:toureventconfig}
\end{figure}

\begin{enumerate}
\item Flag de activación: Permite habilitar y deshabilitar la generación automática de \emph{Tour Events}.
\item Días por adelantado: Permite especificar el número de días desde la fecha actual para calcular hasta qué fecha deben crearse los \emph{Tour Events}.
\end{enumerate}
...
\begin{enumerate}
\item \emph{Duration}: Se debe especificar la duración del tour en minutos.
\item \emph{Blocked Before}: Se debe especificar en minutos, el tiempo que el guía debe tener disponible inmediatamente anterior al tour, en caso de ser necesario.
\item \emph{Blocked After}: Se debe especificar en minutos, el tiempo que el guía debe tener disponible inmediatamente posterior al tour, en caso de ser necesario.
\item \emph{Monday - Sunday Times}: Para cada día de la semana que el tour se ofrezca, se deben listar separados por coma, los horarios de salida en formato hh:mm.
\end{enumerate}

\begin{figure}
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/product_attribute_time}
  \caption{Generación \emph{Tour Events}: Atributos de producto}
  \label{fig:product_attribute_time}
\end{figure}

\item \textbf{Servicio de generación:}

Se ha desarrollado un servicio que se encarga de la generación de \emph{Tour Events}, implementado en la clase~Dbtours\textbackslash{TourEvent}\textbackslash{Service}\textbackslash{Generator}.
Este servicio admite como parámetro de forma opcional un listado de identificadores de producto para los que se deben regenerar los \emph{Tour Events} asociados.
La ejecución del servicio sigue el flujo mostrado en la figura~\ref{fig:toureventdiagram} y como resultado el contenido de la tabla \emph{tour\_event} sería semejante a los datos mostrados en la figura~\ref{fig:toureventtable}

\begin{figure}
  \centering
  \includegraphics[width=10cm, keepaspectratio]{img/tourevent_diagram}
  \caption{Generación \emph{Tour Events}: Diagrama de flujo}
...
\begin{figure}
  ...
  \caption{Contenido de la tabla \emph{tour\_event}}
...
Se ha definido una tarea programada que invoca a este servicio una vez al mes, con la siguiente expresión cron: [\textit{0 0 1 * *}]. La llamada al servicio de generación desde el proceso cron se realiza sin parámetros, por lo que regenerará todos los \emph{Tour Events} de todos los productos.
\end{itemize}

\subsection{Vista tour\_event\_language} 
\label{sec:tourevents}
La entidad \emph{Tour Event Language} consiste en una vista de base de datos, que tiene como objetivo simplificar la consulta de disponibilidad. El objetivo es que dado un producto, sea sencillo listar todas las opciones de fechas e idiomas tanto disponibles como ocupadas.

Su contenido mostrará una entrada única para cada \emph{tour\_event\_id} e idioma ofrecido por algún guía, y contendrá información sobre la disponibilidad y los guías disponibles ordenados por prioridad de asignación. 
Se puede ver en la figura~\ref{fig:toureventlanguage_table} 

En la generación de esta vista también se tiene en cuenta el valor 3 definido en la configuración mostrado en la figura~\ref{fig:toureventconfig}, que permite al administrador definir el tiempo mínimo necesario entre el momento de reserva y el comienzo del tour, de forma que solo se generan entradas en la vista para \emph{Tour Events} que cumplan este margen previo de reserva.

Respecto a la disponibilidad, se considera que un \emph{Tour Event} está disponible cuando existe al menos un guía con disponibilidad en su calendario, es decir, que no exista ninguna entrada en la tabla \emph{calendar\_event}para el guía en cuestión, en el periodo de tiempo comprendido entre la hora de inicio menos el bloqueo anterior y la hora de finalización más el bloqueo posterior del \emph{Tour Event}. 
En el diagrama de la figura~\ref{fig:toureventavailable} se puede ver claramente los casos considerados como \emph{Tour Event} no disponible.
...
El código implementado puede encontrarse en el fichero \textit{Dbtours/TourEvent/Setup/Recurring.php} \footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/TourEvent/Setup/Recurring.php}}. El método principal que se encarga de la generación de la vista es el mostrado en la figura~\ref{fig:toureventlanguage_code} (Apéndice~\ref{app:code}).


\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/toureventlanguage_table}
  \caption{Contenido de la tabla \emph{tour\_event\_language}}
...
\begin{figure}
  ...
  \caption{\emph{Tour Event}: Casos de no disponibilidad}
...
\label{sec:disponibilidad}
Desde la página de detalle de cada tour de la aplicación y gracias a la opción personalizada \emph{tour event}, definida en la sección~\ref{subsec:customoptions}, el usuario podrá consultar la disponibilidad de fecha, hora e idioma en un único paso.
...
\subsection{Validación de disponibilidad} 
\label{sec:validacion-disponibilidad}
Cuando el usuario añade un producto al carrito, es necesario realizar una validación que compruebe que la opción seleccionada por el usuario en el front sigue estando disponible.
Gracias a que el evento \textit{sales\_quote\_item\_set\_product} de Magento se lanza tras dicha acción, se ha definido un nuevo \textit{observer} atado a este evento que se encarga que realizar una llamada a la lógica desarrollada de validación.

Esta lógica se encuentra implementada en los ficheros:
\begin{enumerate}
\item\textit{Dbtours/Catalog/etc/events.xml}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Catalog/etc/events.xml}}: Configuración del \emph{observer} asociado al evento. El código implementado puede verse en la figura~\ref{fig:availability_event} (Apéndice~\ref{app:code}).

\item\textit{Dbtours/Catalog/Observer/AvailabilityValidatorObserver.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Catalog/Observer/AvailabilityValidatorObserver.php}}: Definición del \textit{observer} que se ejecuta al lanzarse el evento y realiza la llamada al método \textit{validate()} de la clase \textit{Dbtours\textbackslash{Catalog}\textbackslash{Model}Quote\textbackslash{Item}AvailabilityValidator}.

\item\textit{Dbtours/Catalog/Model/Quote/Item/AvailabilityValidator.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Catalog/Model/Quote/Item/AvailabilityValidator.php}}: Clase que comprueba si el producto añadido tiene la opción personalizada \textit{tour\_event} y realiza la llamada al método \textit{validateAvailability()} de la clase \textit{Dbtours\textbackslash{TourEvent}\textbackslash{Helper}Validator}.

\item\textit{Dbtours/TourEvent/Helper/Validator.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/TourEvent/Helper/Validator.php}}: Clase que comprueba la disponibilidad en la vista \textit{tour\_event\_language} para la opción seleccionada por el usuario. El código implementado puede verse en la figura~\ref{fig:validator} (Apéndice~\ref{app:code}).
\end{enumerate} 


\subsection{Creación de la reserva} 
\label{sec:reserva}
Después de que el tour ha sido añadido al carrito, el proceso de compra sigue el flujo propio de Magento y el usuario será redirigido al carrito, como se muestra en la figura~\ref{fig:cart}.
Una vez el usuario revisa que su selección es correcta, mediante el botón \emph{``Proceed to Checkout''}, llegará al último paso donde podrá revisar el detalle de su pedido, completar su datos, seleccionar el método de pago y confirmar el pedido como se muestra en la figura~\ref{fig:checkout}.

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/cart}
  \caption{Proceso de compra: \emph{Cart}}
  \label{fig:cart}
\end{figure}
...
\begin{figure}
  ...
  \caption{Proceso de compra: \emph{Checkout}}
  \label{fig:checkout}
\end{figure}

Gracias a que el evento \textit{sales\_model\_service\_quote\_submit\_success} de Magento se lanza en el momento en el que el pedido se realiza correctamente, se ha definido un nuevo \textit{observer} atado a este evento que realizará la creación de la reserva y de los eventos de calendario necesarios. 

Esta lógica se encuentra implementada en los ficheros:

\begin{enumerate}
\item\textit{Dbtours/Sales/etc/events.xml}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Sales/etc/events.xml}}: Configuración del \emph{observer} asociado al evento. El código implementado puede verse en la figura~\ref{fig:success_event} (Apéndice~\ref{app:code}). 

\item\textit{Dbtours/Sales/Observer/OrderObserver.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Sales/Observer/OrderObserver.php}}: Definición del \textit{observer} que se ejecuta al lanzarse el evento y para cada producto contenido en el pedido realiza la llamada al método \textit{execute()} de la clase \textit{Dbtours\textbackslash{Sales}\textbackslash{Service}\textbackslash{Order}NewItem}.

\item\textit{Dbtours/Sales/Service/Order/NewItem.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Sales/Service/Order/NewItem.php}}: Clase que se encarga de realizar la llamada al método \textit{addNewBooking()} de la clase \textit{Dbtours\textbackslash{Booking}\textbackslash{Service}BookingManager} y al método \textit{addCalendarEvents()} de la clase \textit{Dbtours\textbackslash{Calendar}\textbackslash{Service}CalendarManager}. 

\item\textit{Dbtours/Booking/Service/BookingManager.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Booking/Service/BookingManager.php}}: Clase que se encarga de crear una nueva reserva asignada al guía disponible con mayor prioridad. Se insertará una nueva entrada en la tabla \textit{booking} de base de datos.
El código implementado puede verse en las figuras~\ref{fig:booking_manager1} y~\ref{fig:booking_manager2} (Apéndice~\ref{app:code}). 

\item\textit{Dbtours/Calendar/Service/CalendarManager.php}\footnote{\url{https://github.com/rebemartinezgr/dbtours/blob/dev/app/code/Dbtours/Calendar/Service/CalendarManager.php}}: Clase que se encarga de crear los eventos de calendario necesarios para el guía asociado a la reserva. Siempre se creará al menos un evento del tipo \textit{Booking}, y opcionalmente eventos de calendario del tipo \textit{Transfer}, dependiendo de si el tour reservado tenía bloqueos de disponibilidad anterior y/o posterior. Estos bloqueos son el tiempo necesario que se reserva al guía para el desplazamiento antes y después del tour.
Estos eventos de calendario se insertarán como nuevas entradas en la tabla \textit{calendar\_event} de la base de datos.
El código implementado puede verse en la figura~\ref{fig:calendar_manager1},~\ref{fig:calendar_manager2} y~\ref{fig:calendar_manager3} (Apéndice~\ref{app:code}). 
...
\subsection{Creación manual de \emph{Tour Events}} 
\label{sec:tourventmanual}

Aunque no es lo habitual, en ocasiones es posible que el administrador decida cambiar los horarios de inicio de algún tour, su duración, o bien el tiempo de bloqueo anterior y/o posterior. Como ya se mencionó en la sección~\ref{sec:tourevents} estos atributos de producto se utilizan en la generación automática de \emph{Tour Events}, la cual está programada para ejecutarse mensualmente, por lo que si alguno de estos valores cambia para algún tour, estos cambios no se tendrán en cuenta hasta la siguiente generación automática de \emph{Tour Events}.

Para ello se ha añadido una nueva vista en el panel de administración a la que se accede desde la entrada ``Tour Events - Generation'',  cuya función es permitir al administrador lanzar la regeneración de \emph{Tour Events} para uno o varios tours, de forma manual. La vista puede verse en la figura~\ref{fig:toureventmanual}.

Cuando el administrador hace clic en el botón ``Generate'', se invocará al servicio descrito en la sección~\ref{sec:tourevents}, pasándole como parámetros los tours seleccionados, regenerando así todos los \emph{Tour Events} asociados a dichos tours.

\begin{figure}
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/toureventmanual}
  \caption{Generación manual de \emph{Tour Events}}
...
Este \emph{observer} se encargará de realizar las acciones necesarias tras la modificación de una reserva, que pueden ser:
...
\begin{figure}
  ...
  \caption{Tabla \emph{calendar\_event\_type}}
  \label{fig:calandareventtype}
\end{figure}

\subsection{Visualización de eventos de calendario} 
\label{sec:eventoscalendariovisualizacion}
...
\section{Valoración del Cliente} 
\label{sec:valoracion-cliente}
En esta sección se presenta la valoración obtenida del cliente, quien me trasladó éstas impresiones:

\setlength{\leftskip}{1cm}

 \emph{``Gracias a la nueva página web mejoraremos tanto en imagen como a nivel funcional.}

 \emph{A primera vista, la nueva web es más intuitiva y fácil de usar que la anterior. La estructura y organización de la misma, hace que llegues a la información y al proceso final de compra de manera sencilla.}

 \emph{Esta posibilidad de comprar los tours online y las mejoras en el posicionamiento, hará que ganemos nuevos clientes al ofrecer además seguridad en el pago, varias modalidades de pago a los más indecisos y ahorro de tiempo en la compra (evitando tener que contactar cada vez que se quiere comprar un tour como ocurre en este momento), hará también que fidelicemos clientes que ya han estado con nosotros previamente, a través del envío de ofertas, descuentos y boletín de novedades.}

 \emph{Además, mejoraremos en credibilidad al poder consultar referencias, opiniones y enlaces con redes sociales.}

 \emph{Nos supondrá también un ahorro de tiempo al poder gestionar la página web y sus contenidos sin necesidad de contar con un desarrollador, así como un ahorro en gestiones como: la creación y envío de facturas (actualmente a mano), seguimiento rápido y actualizado en todo momento de los pedidos, control de documentos legales\ldots además de la organización de los guías y sus calendarios.}

 \emph{Gracias al proyecto, también hemos obtenido una visión general de nuestra empresa y de los servicios que ofrecemos, al tener que recopilar, estructurar y plasmar toda la información en un Excel de trabajo compartido.}

 \emph{En definitiva, gracias a la dedicación y el trabajo de Rebeca pasaremos de tener una idea futura a una realidad\ldots ¡Mil gracias!''}
...
Durante el desarrollo de este proyecto, se han completado las tareas definidas tanto en la fase I como en la fase II de la sección \ref{sec:metodologia-trabajo}.
...
Gran parte del desarrollo de este proyecto ha necesitado una comunicación continua con el cliente, al cual a pesar de estar dispuesto de ayudar en todo lo posible, ha sido necesario guiar en la definición de necesidades.
En ocasiones, a medida que ha ido avanzando el desarrollo han ido surgiendo nuevos requisitos y ha sido necesario revisar el planteamiento inicial.
A continuación se presentan algunos ejemplos.

\subsubsection{Definición de los horarios de los tours}
El proceso de la definición de los horarios de los tours ha ido evolucionando durante el desarrollo. 
Inicialmente se pensó en que todos los tours se ofrecerían a las mismas horas y los mismos días de la semana, por lo que se añadió una configuración común a todos ellos.
Posteriormente nació la necesidad de que cada tour pudiera tener sus propios horarios por lo que fue necesario trasladar esa información a nivel de atributo de producto. 
Finalmente con el último análisis del catálogo descubrí que algunos tours necesitaban que el guía estuviera disponible con anterioridad o posterioridad a la realización del tour, debido al tiempo necesario de traslado. Dado que el tiempo de traslado variaba según el tour, se decidió añadir dos atributos más para contemplar éstos dos casos y fue necesario añadir estos valores en varias de las entidades de base de datos para que se tuvieran en cuenta en la consulta de disponibilidad.  

\subsubsection{Definición de los idiomas}
Inicialmente se pensó que los idiomas en los que se ofrecerían los tours serían los mismos idiomas en los que se ofrecía la interfaz de la aplicación web (Alemán, Español e Inglés) por lo que el usuario no tendría la opción de seleccionar el idioma en la reserva del tour. 
Posteriormente nos dimos cuenta que desvinculando los idiomas de los tours de los idiomas de la aplicación, en el futuro sería más fácil incluir nuevo idiomas en los que ofrecer los tours, por lo que finalmente se decidió que el idioma ofrecido estaría ligado el propio guía y se añadió la opción para que el usuario pudiera seleccionar el idioma en el desea realizar el tour.
...
A continuación se muestran los enlaces principales tanto a la aplicación como a los repositorios que contienen el código y la presente memoria.

 \begin{itemize}
  \item Dominio de la aplicación. \footnote{\url{http://dev.dbcntours.com/}}
 \item Repositorio con el código fuente de la aplicación (Rama dev). \footnote{\url{https://github.com/rebemartinezgr/dbtours}}
 \item Repositorio de la memoria. \footnote{\url{https://github.com/rebemartinezgr/MemoriaTFG-RebecaMartinezGarcia}}
 \item Presente documento generado. \footnote{\url{https://github.com/rebemartinezgr/MemoriaTFG-RebecaMartinezGarcia/blob/master/memoria.pdf}}
\end{itemize}

\chapter{Código}
\label{app:code}
A continuación se presentan algunos de los fragmentos de código más importantes en la implementación de la aplicación.

\begin{figure}
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/toureventlanguage_code}
  \caption{Generación de la vista \emph{tour\_event\_language}}
  \label{fig:toureventlanguage_code}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/availability_event}
  \caption{Configuración \emph{Availability Observer}}
  \label{fig:availability_event}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/validator}
  \caption{Clase \emph{Validator}}
  \label{fig:validator}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/success_event}
  \caption{Configuración \emph{Order Success Observer}}
  \label{fig:success_event}
\end{figure}


\begin{figure}
  \centering
  \includegraphics[width=15cm, keepaspectratio]{img/booking_manager1}
  \caption{Clase \emph{BookingManager} 1/2}
  \label{fig:booking_manager1}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/booking_manager2}
  \caption{Clase \emph{BookingManager} 2/2}
  \label{fig:booking_manager2}
\end{figure}


\begin{figure}
  \centering
  \includegraphics[width=15cm, keepaspectratio]{img/calendar_manager1}
  \caption{Clase \emph{CalendarManager} 1/3}
  \label{fig:calendar_manager1}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=15cm, keepaspectratio]{img/calendar_manager2}
  \caption{Clase \emph{CalendarManager} 2/3}
  \label{fig:calendar_manager2}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=15cm, keepaspectratio]{img/calendar_manager3}
  \caption{Clase \emph{CalendarManager} 3/3}
  \label{fig:calendar_manager3}
\end{figure}
...
Una vez contratado el servidor y con acceso ssh, han sido necesarios los siguientes pasos para la configuración del sistema y despliegue de la aplicación:
