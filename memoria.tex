...
Los datos de conexión de la aplicación con la base de datos, se definen en el archivo de configuración env.php que se encuentra localizado en la ruta app/etc/env.php del código.
...
El código implementado puede encontrarse en el fichero \textit{Dbtours/TourEvent/Setup/Recurring.php}. El método principal que se encarga de la generación de la vista es el mostrado en la figura~\ref{fig:toureventlanguage_code}
...
\begin{figure}
  ...
  \caption{Generación de la vista tour\_event\_language}
...
\begin{enumerate}
\item\textit{Dbtours/Catalog/etc/events.xml}: Configuración del observer asociado al evento. El código implementado puede verse en la figura~\ref{fig:availability_event}.

\item\textit{Dbtours/Catalog/Observer/AvailabilityValidatorObserver.php}: Definición del \textit{observer} que se ejecuta al lanzarse el evento y realiza la llamada al método \textit{validate()} de la clase \textit{Dbtours\textbackslash{Catalog}\textbackslash{Model}Quote\textbackslash{Item}AvailabilityValidator}. El código implementado puede verse en la figura~\ref{fig:availability_validator_observer}.

\item\textit{Dbtours/Catalog/Model/Quote/Item/AvailabilityValidator.php}: Clase que comprueba si el producto añadido tiene la opción personalizada \textit{tour\_event} y realiza la llamada al método \textit{validateAvailability()} de la clase \textit{Dbtours\textbackslash{TourEvent}\textbackslash{Helper}Validator}. El código implementado puede verse en la figura~\ref{fig:availability_validator}.

\item\textit{Dbtours/TourEvent/Helper/Validator.php}: Clase que comprueba la disponibilidad en la vista \textit{tour\_event\_language} para la opción seleccionada por el usuario. El código implementado puede verse en la figura~\ref{fig:validator}
\end{enumerate} 

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/availability_event}
  \caption{Configuración Availability Observer}
  \label{fig:availability_event}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=10cm, keepaspectratio]{img/availability_validator_observer}
  \caption{Clase AvailabilityValidatorObserver}
  \label{fig:availability_validator_observer}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/availability_validator}
  \caption{Clase AvailabilityValidator}
  \label{fig:availability_validator}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=16cm, keepaspectratio]{img/validator}
  \caption{Clase Validator}
  \label{fig:validator}
\end{figure}
...
\begin{enumerate}
\item\textit{Dbtours/Sales/etc/events.xml}: Configuración del observer asociado al evento. El código implementado puede verse en la figura~\ref{fig:success_event}.

\item\textit{Dbtours/Sales/Observer/OrderObserver.php}: Definición del \textit{observer} que se ejecuta al lanzarse el evento y para cada producto contenido en el pedido realiza la llamada al método \textit{execute()} de la clase \textit{Dbtours\textbackslash{Sales}\textbackslash{Service}\textbackslash{Order}NewItem}. El código implementado puede verse en la figura~\ref{fig:order_observer}.

\item\textit{Dbtours/Sales/Service/Order/NewItem.php}: Clase que se encarga de realizar la llamada al método \textit{addNewBooking()} de la clase \textit{Dbtours\textbackslash{Booking}\textbackslash{Service}BookingManager} y al método \textit{addCalendarEvents()} de la clase \textit{Dbtours\textbackslash{Calendar}\textbackslash{Service}CalendarManager} . El código implementado puede verse en la figura~\ref{fig:new_item}.

\item\textit{Dbtours/Booking/Service/BookingManager.php}: Clase que se encarga de crear una nueva reserva asignada al guía disponible con mayor prioridad. Se insertará una nueva entrada en la tabla \textit{booking} de base de datos.
El código implementado puede verse en las figuras~\ref{fig:booking_manager1} y~\ref{fig:booking_manager2} .

\item\textit{Dbtours/Calendar/Service/CalendarManager.php}: Clase que se encarga de crear los eventos de calendario necesarios para el guía asociado a la reserva. Siempre se creará al menos un evento del tipo \textit{Booking}, y opcionalmente eventos de calendario del tipo \textit{Transfer}, dependiendo de si el tour reservado tenía bloqueos de disponibilidad anterior y/o posterior. Estos bloqueos son el tiempo necesario que se reserva al guía para el desplazamiento antes y después del tour.
Estos eventos de calendario se insertarán como nuevas entradas en la tabla \textit{calendar\_event} de la base de datos.
El código implementado puede verse en la figura~\ref{fig:calendar_manager1},~\ref{fig:calendar_manager2} y~\ref{fig:calendar_manager3}.
...
\begin{figure}
  ...
  \caption{Configuración Order Success Observer}
  \label{fig:success_event}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=12cm, keepaspectratio]{img/order_observer}
  \caption{Clase Order Observer}
  \label{fig:order_observer}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=14cm, keepaspectratio]{img/new_item}
  \caption{Clase NewItem}
  \label{fig:new_item}
\end{figure}
...
\begin{figure}
  ...
  \caption{Clase BookingManager 1/2}
...
\begin{figure}
  ...
  \caption{Clase BookingManager 2/2}
...
\begin{figure}
  ...
  \caption{Clase CalendarManager 1/3}
...
\begin{figure}
  ...
  \caption{Clase CalendarManager 2/3}
...
\begin{figure}
  ...
  \caption{Clase CalendarManager 3/3}
